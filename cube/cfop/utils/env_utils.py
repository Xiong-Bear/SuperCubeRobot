...
def get_environment(env_name: str) -> Environment:
    ...
    if env_name == 'cube3':
        from cube.cfop.environments.cube3 import Cube3
        env = Cube3()
    elif env_name == 'cube3_cross':
        from cube.cfop.environments.cube3_cross import Cube3_Cross
        env = Cube3_Cross()
    elif env_name == 'cube3_f2l':
        from cube.cfop.environments.cube3_f2l import Cube3_First2Layers
        env = Cube3_First2Layers()
    elif puzzle_n_regex is not None:
        from cube.cfop.environments.n_puzzle import NPuzzle
        puzzle_dim: int = int(math.sqrt(int(puzzle_n_regex.group(1)) + 1))
        env = NPuzzle(puzzle_dim)
    elif 'lightsout' in env_name:
        from cube.cfop.environments.lights_out import LightsOut
