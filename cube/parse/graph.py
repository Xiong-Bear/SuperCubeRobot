...
def judgeColor(hsv):
    ...
    if 255 >= s >= 24 <= h <= 45 <= v <= 255:
        return 'Y'
    elif 0 <= h <= 180 and 105 >= s >= 0 <= v <= 255:
        return 'W'
    elif 156 <= h <= 180 and 255 >= s >= 43 <= v <= 255:
        return 'R'
    elif 0 <= h <= 5 and 255 >= s >= 43 <= v <= 255:
        return 'R'
    elif 255 >= s >= 45 <= h <= 86 <= v <= 255:
        return 'G'
    # elif h>=170 and h<=180 and s>=43 and s<=255 and v>=46 and v<=255:
    # return 'O'
    elif 6 <= h <= 23 and 43 <= s <= 255 and 46 <= v <= 255:
        return 'O'
    elif 255 >= s >= 100 <= h <= 124 <= v <= 255:
        return 'B'
...
def showTem2(tempList, frame):
    l = 30
    location = [[46, 101, 74, 129], [46, 221, 74, 249], [46, 341, 74, 369], [440, 101, 468, 129], [440, 221, 468, 249],
                [440, 341, 468, 369]]
    x = [0, 1, 2, 0, 1, 2, 0, 1, 2]
    y = [0, 0, 0, 1, 1, 1, 2, 2, 2]
    for i in location:
        for j in range(9):
            ...
            cv2.rectangle(frame, (i[0] + l * x[j], i[1] + l * y[j]), (i[2] + l * x[j], i[3] + l * y[j]),
                          (RGB[2], RGB[1], RGB[0]), -1)
...
def findLocation(xx, yy):
    ...
    for i in location:
        for j in range(9):
            if i[0] + l * x[j] <= xx <= i[2] + l * x[j] >= yy >= i[1] + l * y[j]:
